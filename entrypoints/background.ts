import { startRegionCapture } from "./background/region";
...
export default defineBackground(() => {
  ...
  async function handleRegionTabReady(
    message: any,
    sender: chrome.runtime.MessageSender
  ) {
    ...
    try {
      ...
      if (!delivered) {
        console.log(
          "📦 No extension contexts available, storing region tab ready state"
        );
  ...
  async function handleRegionSelected(
    ...
    try {
      ...
      if (!delivered) {
        ...
        const resultId = `region_${Date.now()}_${Math.random()
          .toString(36)
          .substr(2, 9)}`;
        await chrome.storage.local.set({
  ...
  function handleRegionCancelled(
    message: any,
    sender: chrome.runtime.MessageSender
  ) {
  ...
  async function handleRegionSelectorData(
    message: any,
    sender: chrome.runtime.MessageSender
  ) {
    ...
    try {
      ...
      if (regionSelectorTabs.length > 0) {
        ...
        return {
          delivered: true,
          method: "region_tabs",
          count: regionSelectorTabs.length,
        };
      }
  ...
  async function handlePopupOpened(sender: chrome.runtime.MessageSender) {
    ...
    try {
      ...
      const videoStorage = await chrome.storage.local.get([
        "has_pending_video_results",
        "latest_video_result_id",
      ]);

      if (
        videoStorage.has_pending_video_results &&
        videoStorage.latest_video_result_id
      ) {
      ...
      const regionStorage = await chrome.storage.local.get([
        "has_pending_region_result",
        "latest_region_result_id",
      ]);

      if (
        regionStorage.has_pending_region_result &&
        regionStorage.latest_region_result_id
      ) {
        ...
        if (result[resultKey]) {
          ...
          setTimeout(async () => {
            try {
              ...
              console.error(
                "❌ Error delivering pending region result:",
                error
              );
            }
          }, 1500);
      ...
      for (const [key, value] of Object.entries(storage)) {
        if (
          (key.startsWith("pending_video_") ||
            key.startsWith("pending_region_")) &&
          typeof value === "object" &&
          value !== null
        ) {
